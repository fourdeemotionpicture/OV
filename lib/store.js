...
const FALLBACK_DEFAULT_KEY = ['Admin', '@', 'OV', '2026', '!'].join('');

function getDefaultState() {
  const defaultAdminPassword = hashPassword(process.env.ADMIN_INITIAL_PASSWORD || FALLBACK_DEFAULT_KEY);
  ...
  return {
    ...
    variants: [
      ...
      { id: 'var-noir-s', product_id: 'prod-noir-black', sku: 'OV-NOIR-BLK-S', size: 'S', color: 'Washed Black', price: 1199, stock_quantity: 25, reserved_quantity: 0, low_stock_threshold: 5 },
      { id: 'var-noir-m', product_id: 'prod-noir-black', sku: 'OV-NOIR-BLK-M', size: 'M', color: 'Washed Black', price: 1199, stock_quantity: 40, reserved_quantity: 0, low_stock_threshold: 5 },
      { id: 'var-noir-l', product_id: 'prod-noir-black', sku: 'OV-NOIR-BLK-L', size: 'L', color: 'Washed Black', price: 1199, stock_quantity: 35, reserved_quantity: 0, low_stock_threshold: 5 },
      { id: 'var-noir-xl', product_id: 'prod-noir-black', sku: 'OV-NOIR-BLK-XL', size: 'XL', color: 'Washed Black', price: 1199, stock_quantity: 20, reserved_quantity: 0, low_stock_threshold: 5 }
    ],
    orders: [
      {
        id: 'ord-10001',
...
class EmbeddedStore {
  ...
  load() {
    try {
      if (fs.existsSync(STORE_FILE)) {
        ...
        const seedPath = path.join(process.cwd(), 'data', 'ov_store.json');
      ...
      const activePassword = process.env.ADMIN_INITIAL_PASSWORD || FALLBACK_DEFAULT_KEY;
      const defaultHash = hashPassword(activePassword);

      if (!this.state || !Array.isArray(this.state.admin_users) || this.state.admin_users.length === 0) {
        this.state = getDefaultState();
      } else {
        this.state.admin_users.forEach(u => {
          if (!u.password_hash) {
      ...
      if (!this.state.settings) {
        this.state.settings = getDefaultState().settings;
      } else {
        this.state.settings = { ...getDefaultState().settings, ...this.state.settings };
      }
    } catch (e) {
  ...
  updateVariantStock(variantId, { stock_quantity, delta, price }) {
    ...
    if (stock_quantity !== undefined) {
      ...
      if (parsedQty < 0) throw new Error('Stock quantity cannot be negative');
      variant.stock_quantity = parsedQty;
    } else if (delta !== undefined) {
      const parsedDelta = parseInt(delta, 10);
      if (variant.stock_quantity + parsedDelta < 0) {
        throw new Error(`Insufficient stock for variant ${variant.sku}. Current: ${variant.stock_quantity}, Delta: ${parsedDelta}`);
      }
  ...
  getOrderById(id) {
    ...
    return this.state.orders.find(o => (o.id && String(o.id).toUpperCase() === searchId) || (o.order_number && String(o.order_number).toUpperCase() === searchId));
  }
  ...
  updateSettings(newSettings) {
    if (!newSettings || typeof newSettings !== 'object') return this.state.settings;

    this.state.settings = { ...this.state.settings, ...newSettings };

    // Synchronize custom products into store products catalog and variants
    if (Array.isArray(newSettings.custom_products) && newSettings.custom_products.length > 0) {
      newSettings.custom_products.forEach(cp => {
        if (!cp || !cp.id) return;
        const existingIdx = this.state.products.findIndex(p => p.id === cp.id);
        const prodItem = {
          id: cp.id,
          title: cp.name || cp.baseName || 'OV Streetwear Garment',
          slug: (cp.baseName || cp.name || cp.id).toLowerCase().replace(/[^a-z0-9]+/g, '-').replace(/(^-|-$)/g, ''),
          description: cp.desc || '',
          category_id: 'cat-tees',
          brand: cp.brand || 'OV — ORIGINAL VERSION',
          base_price: cp.price || 999,
          compare_at_price: cp.originalPrice || 2999,
          cost_price: Math.round((cp.price || 999) * 0.45),
          status: 'ACTIVE',
          is_featured: true,
          weight_grams: 450,
          images: Array.isArray(cp.gallery) && cp.gallery.length > 0 ? cp.gallery : [cp.image || 'images/product_beige_front_model.jpg'],
          attributes: {
            fit: cp.fit || 'Oversized Boxy Fit',
            fabric: cp.fabric || '240 GSM Luxury Combed Cotton',
            neck: 'High-Density Ribbed Collar',
            origin: 'Crafted in Tirupur, India'
          },
        ...
        if (existingIdx !== -1) {
          this.state.products[existingIdx] = { ...this.state.products[existingIdx], ...prodItem };
        } else {
          this.state.products.unshift(prodItem);
        }

        // Ensure variants exist for inventory tracking
        const sizes = Array.isArray(cp.sizes) && cp.sizes.length > 0 ? cp.sizes : ['M', 'L'];
        sizes.forEach(sz => {
          const varId = `var-${cp.id}-${sz.toLowerCase()}`;
          if (!this.state.variants.some(v => v.id === varId)) {
            this.state.variants.push({
              id: varId,
              product_id: cp.id,
              sku: `OV-${cp.id.slice(-4).toUpperCase()}-${sz}`,
              size: sz,
              color: 'Classic',
              price: cp.price || 999,
              stock_quantity: cp.stock || 15,
              reserved_quantity: 0,
