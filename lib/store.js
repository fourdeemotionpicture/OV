...
function getDefaultState() {
  const defaultAdminPassword = process.env.ADMIN_INITIAL_PASSWORD
    ? hashPassword(process.env.ADMIN_INITIAL_PASSWORD)
  ...
  load() {
    try {
      if (fs.existsSync(STORE_FILE)) {
        ...
        if (process.env.ADMIN_INITIAL_PASSWORD && this.state.admin_users) {
          const freshHash = hashPassword(process.env.ADMIN_INITIAL_PASSWORD);
          this.state.admin_users.forEach(u => { u.password_hash = freshHash; });
        }
      } else {
